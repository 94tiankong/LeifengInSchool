...
Page({
  ...
  onLoad: function (options) {
    this.setData({
      ...
      seekeravatar: options.seekeravatar
    })
  },
  //点两次相同分数取消选择 js
  selectIndexNum(e) {
  ...
  sbmitFeedback: function (e) {
    ...
    var starVal=that.data.starVal+1;
    starVal = starVal * 2 ;
    ...
    //对求助者评价
    util.request(api.CommentSeeker, { helpId: that.data.helpId, starVal: starVal, comment: that.data.content }).then(function (res) {
      console.log(res)
      if (res.errno === 0) {
        wx.hideLoading();
        wx.showToast({
        ...
        setTimeout(function(){
          wx.navigateTo({
            url: '/pages/grabbedList/grabbedList'
          })
        },1000);
